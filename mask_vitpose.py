...
import cv2
from easy_ViTPose import VitInferenceWithMask
...
def main(args):
    ...
    model = VitInferenceWithMask(model_path, args.scene[0], model_name='l', is_video=True, device=None,
    ...
    for scene in args.scene:
        for view in args.view:
            ...
            frame_id_list = list(
                range(args.start, args.end if args.end != -1 else int(capture.get(cv2.CAP_PROP_FRAME_COUNT)),
                      args.step))
            # threading.Thread(target=read_images, args=(image_base, frame_id_list, q_read)).start()
            model.udpate_mask_path(os.path.join(scene, "new_masks", view))
